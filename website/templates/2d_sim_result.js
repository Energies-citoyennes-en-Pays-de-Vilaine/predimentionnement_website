...
function sendRequest(url, requestParams, callback){
	...
	xhr.onload = function(){
		...
		console.log({url,params_url, response:xhr.response})
	}
...
function updateChart(data, name, labels = {}){
	...
	console.log({data,x_data,y_data})
...
function actualize(params){
	return function(){
		...
		varying_index_pos = fixed_indexes.indexOf(values["varying_index"])
		if (varying_index_pos != -1)
		{
			fixed_indexes.splice(varying_index_pos, 1)
			fixed_values.splice(varying_index_pos, 1)
		}
		values["fixed_indexes"] = fixed_indexes
		values["fixed_values"]  = fixed_values
		sendRequest("/sims/api/results/vdata", values, callback )
	}
}
...
window.addEventListener("load", function(){
	sendRequest("/sims/api/results/index_result", {}, (response)=>{
		...
		sendRequest("/sims/api/results/index", {}, (response)=>{
			...
			sendRequest("/sims/api/results/vdata", default_data, callback )
			...
			[
				window.setEnumParam("varying_index", "paramètre à faire varier", indexes, defaultValue = 0, formType = "radio"),
				...selectParams,
